...
class ClassificationDatasetPreparation(object):
    ...
    def __init__(
        ...
        self.dirs = os.listdir(self.label_data_root)#当前具体文件夹名称
        self.num_classes = 0

    def gen_annotations(self, dataset_path):
        for idx in tqdm.tqdm(range(len(self.dirs))):
            dirname = self.dirs[idx] #当前文件目录
    ...
    def _get_subdir_annotations(self, path):
        ...
        for file in files:
            ...
            if dirname in ['break', 'deformation', 'missing']:
                dirname = 'abnormal'
            annotations.append(f"{file} {dirname}")#制作
...
if __name__ == "__main__":
    ...
    # # task:中车受电弓场景分类
    label_data_root = "/mnt/10_AlgorithmData/ZNJT/ZhongChe_sdg_scene/CleanData"
    keyword = [".jpg", ".png"]
    ...
    task_name = 'ZhongChe_sdg_scene'

    dataset_path = os.path.join('./tasks', task_name, 'dataset')
    make_if_not_exit(dataset_path) #生成路径
