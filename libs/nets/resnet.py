...
def resnet18_(pretrained=False, **kwargs):
    ...
    """
    model = ResNet_(BasicBlock, [2, 2, 2, 2], **kwargs)
  #  if pretrained:
...
class ResNet18(nn.Module):
    ...
    def __init__(self, num_classes=2, pretrained=True):
       ...
       # self.resnet18.conv1=nn.Conv2d(3, 64, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), bias=False)  ##hgx
        self.resnet18.avgpool =nn.AvgPool2d(kernel_size=7, stride=2, padding=0)
...
class ResNet18_480x480(nn.Module):

    def __init__(self, num_classes=2, pretrained=True):
        super(ResNet18_480x480, self).__init__()
        self.num_classes = num_classes
        self.pretrained = pretrained
        self.resnet18 = resnet18_(pretrained=pretrained)
        self.resnet18.fc = nn.Linear(256, 256)
       # self.resnet18.conv1=nn.Conv2d(3, 64, kernel_size=(7, 7), stride=(1, 1), padding=(3, 3), bias=False)  ##hgx
        self.resnet18.avgpool =nn.AvgPool2d(kernel_size=15, stride=2, padding=0)
        self.net_add = nn.Sequential(
            nn.Dropout(0.2),
            nn.ReLU(),
            nn.Linear(256, num_classes)  ##hgx
