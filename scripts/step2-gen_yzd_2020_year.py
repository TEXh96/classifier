...
if __name__ == '__main__':
    ...
    root_path='/mnt/10_data3_zhudao/上海/2020/20201214-上海-数据整理-1/'
    save_path = '/mnt/10_data3_zhudao/loc_result/'#os.path.basename(root_path)
    ...
    for idx in tqdm.tqdm(range(len(dirs))):
        ...
        for img_file in img_files:
            ...
            camID = str(int(os.path.basename(img_file).split("_")[1]))
            print("camID: ",camID)
            # import pdb
            # pdb.set_trace()
            ...
            print("camID_loc: ",camID,str(yzd))
            os.makedirs(path, exist_ok=True)
         
            name = f"{loc}_{camID}_{str(yzd)}_{os.path.basename(img_file)}"
            cv2.imwrite(os.path.join(path, name),img)
            # cv2.imwrite(os.path.join(path, base_name+"_ROI.jpg"),img_roi)
            # path_roi = os.path.join(save_path, loc, camID)
            # os.makedirs(path_roi, exist_ok=True)
            # cv2.imwrite(os.path.join(path_roi, base_name+"_ROI.jpg"),img_roi)
